...
- `hyprland.inc` is the repo's **only AUR dependency**. It bootstraps `yay` (guarded on `command -v yay`) and installs `hyprland-plugin-hy3`, which is compiled against an exact Hyprland version and refuses to load against any other. The repo-available Hyprland packages come from the main pacman list; this file exists only for the AUR part. Sway is deliberately still installed and stowed on Arch as the fallback session for when a `hyprland` upgrade outruns the plugin.
